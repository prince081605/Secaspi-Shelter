...
return [
    ...
    'backup' => [
         ...
         * under on the destination disk, so keep it slug-friendly (no spaces).
         */
        'name' => env('BACKUP_NAME', 'secaspi-shelter'),

        'source' => [
            'files' => [
                /*
                 * We back up the database only — uploaded images already live on R2
                 * (FILESYSTEM_DISK=s3) and the application code is tracked in git. Leaving
                 * this empty keeps each backup small and focused on the irreplaceable data.
                 */
                'include' => [],
        ...
        'destination' => [
            ...
            /*
             * The disk names on which the backups will be stored. Defaults to the R2/S3
             * disk (config/filesystems.php) so dumps survive even if the whole server dies.
             * Set BACKUP_DISK=local in dev to drop dumps in storage/app/private instead.
             */
            'disks' => [
                env('BACKUP_DISK', 's3'),
            ],
     ...
     */
    'notifications' => [
        // Only email on problems — a daily "success" email would be noise. You hear about
        // a failed/missing/unhealthy backup *before* you need to rely on it.
        'notifications' => [
