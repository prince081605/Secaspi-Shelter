...
pages) + live API probes + full PHPUnit suite (**111 green** as audited → **137 green** after the fix
...
> **Done** — §0.1 cleanup · §0.2 backend · HIGH security (auth + public/AI) · HIGH admin pagination (§§3–8) · MED security (CSV-injection, staff-gate, public exposure, AI cap+proxies). **Private-disk uploads (§5/6/7) were attempted then reverted** (ephemeral prod storage → blank proofs); proofs stay on the public R2 disk, re-hardening deferred to a private bucket.
...
> Suite **137 green**; initial JS bundle 1043 kB → 242 kB.
...
| MED security | **✅ CSV-injection (§11)** · **✅ staff-grant gate (§7)** · **✅ public field exposure (§2/3)** · **✅ AI global cap + trust-proxies (§10)**; ⬜ **private-disk uploads (§5/6/7) — attempted then REVERTED.** Donation proofs / rescue photos / intake docs were moved to a private, signed-URL disk, but on Render's free tier that disk was the ephemeral container filesystem (no private R2 bucket set up), so uploads vanished on sleep/redeploy and showed blank in admin. Reverted to the public R2 disk (`Storage::url()`, like animal photos) so they persist + render. Re-hardening needs a separate private R2 bucket — deferred. | ⏳ in progress | suite **137 green**; Appendix A3 #4 |
...
### E. Security
- **[MED] ⬜ DEFERRED — donation proof screenshots stay on the public disk (private-disk attempt
  reverted).** The proof was briefly moved to a private, signed-URL disk, but on Render's free tier
  that disk was the ephemeral container filesystem (no private R2 bucket configured), so proofs
  vanished on sleep/redeploy and rendered blank in the admin view. Reverted: `store('donations')` +
  `Storage::url()` on the public R2 disk (persists + renders, like animal photos). Trade-off: a proof
  is reachable by anyone with the direct URL. Re-hardening needs a separate private R2 bucket served
  via presigned `temporaryUrl()`. _(`DonationController.php`; see PARITY.md.)_
...
- **[MED] ⬜ DEFERRED — rescue photos stay on the public disk (private-disk attempt reverted).**
  Same story as donations §5: the private/signed-URL approach broke on Render's ephemeral free-tier
  storage, so it was reverted to `store('rescue-reports')` + `Storage::url()`. Re-hardening needs a
  private R2 bucket. _(Shared decision with donations §5 and intakes §7 — see PARITY.md.)_
...
- **[MED] ⬜ DEFERRED — intake documents stay on the public disk (private-disk attempt reverted).**
  Same story as donations §5 / rescue §6: reverted to `store('intakes')` + `Storage::url()`; the
  intake→animal conversion is again a same-disk `Storage::copy`. Re-hardening needs a private R2
  bucket. _(Shared decision — see PARITY.md.)_
...
- **[MED] ⬜ DEFERRED — sensitive uploads stay on the public disk.** A private + signed-URL disk was
  built for donation proofs (§5), rescue photos (§6), and intake docs (§7), but it was **reverted**:
  on Render's free tier the private disk was the ephemeral container filesystem (no private R2 bucket
  set up), so uploads vanished on sleep/redeploy and showed blank in admin. All uploads are on the
  public R2 disk (`Storage::url()`). Re-hardening requires a separate **private R2 bucket** served via
  presigned `temporaryUrl()` — deferred until that bucket exists.
...
| **Sensitive uploads on the public disk** (served unauthenticated) — ⬜ DEFERRED: private+signed disk was built then reverted (ephemeral free-tier storage → blank proofs); needs a private R2 bucket to re-harden (§5/6/7) | 5, 6, 7 | MED | Private R2 bucket + signed URLs |
...
**MEDIUM (security)**
4. ⬜ **DEFERRED (attempted + reverted)** — Donation proofs / rescue photos / intake docs were moved
   to a private, signed-URL disk, but on Render's free tier that disk was the ephemeral container
   filesystem (no private R2 bucket configured), so uploads vanished on sleep/redeploy and rendered
   blank in admin. Reverted to the public R2 disk (`Storage::url()`, like animal photos) so they
   persist + render (§5/6/7). To re-harden: create a **separate private R2 bucket** and serve proofs
   via presigned `temporaryUrl()`. See PARITY.md.
